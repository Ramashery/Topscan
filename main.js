...
(function () {
  ...
  if (!prefersReducedMotion) {
    ...
    if (headingTargets.length) {
      if ('IntersectionObserver' in window) {
        /* Play once and stop observing: re-triggering this on every scroll
           direction (including scrolling back up) forced a synchronous
           reflow (playZip -> void el.offsetWidth) on every heading each
           time it crossed the 30% threshold, adding jank right in the
           middle of a scroll gesture. Firing it once on first entry gives
           the same entrance effect without the repeat cost. */
        var headingIO = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) { playZip(entry.target); headingIO.unobserve(entry.target); }
          });
        }, { threshold: 0.3 });
...
(function () {
  ...
  (function () {
    ...
    window.addEventListener('touchmove', function (e) {
      ...
      var p = e.touches[0];
      if (t.axis === null) {
        var dx = p.clientX - t.x0, dy0 = p.clientY - t.y0;
        if (Math.abs(dx) < AXIS_LOCK_PX && Math.abs(dy0) < AXIS_LOCK_PX) return;
        t.axis = Math.abs(dy0) > Math.abs(dx) ? 'y' : 'x';
        if (t.axis === 'x') { t = null; return; }   // horizontal: leave it to whatever else handles it
      }
      var now = performance.now();
      var dy = (t.y - p.clientY) * TOUCH_GAIN;
      /* Already at the very top and still pulling down: hand this one
         gesture back to the browser instead of preventDefault-ing it, so
         its native overscroll/bounce can happen — that's the exact motion
         pull-to-refresh watches for, and it needs un-prevented touchmove
         events to arm. Everywhere else (mid-page, or scrolling down from
         the top) the weighted engine below keeps driving it as before. */
      if (current <= 0.5 && dy < 0) { t = null; return; }
      e.preventDefault();
