...
(function () {
  ...
  (function () {
    ...
    /* Touch devices opt out of this whole engine and get plain native scrolling.
       Two reasons: (1) window.scrollTo() on every touchmove/rAF frame runs on the
       main thread, competing with autoplaying <video> decode/composite work —
       on video-heavy pages (offer pages have two: the hero video and the terrain
       preview) that main-thread contention is what causes the visible stutter;
       native touch scrolling runs on the compositor thread and doesn't have this
       problem. (2) touchmove below unconditionally calls e.preventDefault() once
       the gesture is recognised as vertical, which also silently swallows the
       browser's native "pull down at the top to refresh" gesture. Desktop wheel
       and keyboard scrolling are unaffected and keep the eased/heavier feel. */
    var TOUCH_ENGINE_ENABLED = !(window.matchMedia && window.matchMedia('(pointer: coarse)').matches);

    var t = null;   // active touch, while it's driving the page
    window.addEventListener('touchstart', function (e) {
      if (!TOUCH_ENGINE_ENABLED || locked() || ownsItsScroll(e.target) || e.touches.length !== 1) { t = null; return; }
      resync();
      var p = e.touches[0];
