...
/* =========================================================
   ...
   Hand-rolled FLIP flight (not the native cross-document View
   Transition API — that depends on browser support and internal
   timing we don't fully control, which is why an earlier version of
   this was unreliable).

   1. On leaving the page (vtCapture, called on `pagehide` — covers a
      normal link click AND the browser's own Back/Forward buttons,
      not just one specific trigger link), we measure the on-screen
      rect + border-radius of the current "slot" (.sv-slide on the
      catalog page, .service-hero-visual on the offer page) and store
      it in sessionStorage with the video's src and currentTime. The
      browser then navigates normally.
   2. On the destination page, vtLand() resumes the real <video> at
      the stored currentTime, then — unless reduced motion is on or
      the viewport changed too much — flies *that same real element*
      (not a clone) from the stored rect to its real slot. Using the
      real element means it's showing actual footage the whole time
      (it started loading the moment its own page began parsing, well
      before this script runs) rather than a freshly-created video
      that has to buffer from zero. The flight itself only animates
      `transform` + `border-radius` (GPU-composited), not
      left/top/width/height — animating the box model directly is
      what caused the jank/late-frame "snap" on mobile.
========================================================== */
var VT_KEY = 'topscanVT';
var VT_DURATION = 680;
var VT_EASE = 'cubic-bezier(.76,0,.24,1)';

// Called on `pagehide`, on the "slot" (the element whose rect/radius
// represents the shared video's visible shape) and the actual
// <video> inside it.
function vtCapture(slotEl, videoEl) {
  if (!slotEl || !videoEl) return;
  try {
    var r = slotEl.getBoundingClientRect();
    var radius = parseFloat(getComputedStyle(slotEl).borderRadius) || 0;
    sessionStorage.setItem(VT_KEY, JSON.stringify({
      src: videoEl.currentSrc || videoEl.src,
      t: isFinite(videoEl.currentTime) ? videoEl.currentTime : 0,
...
// Peeks at the flag without consuming it — lets a page check "am I
// about to run a landing flight" before it does its own first-paint
// setup, without prematurely using up the (one-shot) flag.
function vtPeek() {
  try {
    var raw = sessionStorage.getItem(VT_KEY);
    if (!raw) return null;
    var data = JSON.parse(raw);
    if (!data || (Date.now() - data.ts) > 4000) return null;
    return data;
  } catch (e) { return null; }
}

// One-time read: returns the stored data (if still fresh) and clears it.
function vtConsume() {
  var data = vtPeek();
  if (data) { try { sessionStorage.removeItem(VT_KEY); } catch (e) {} }
  return data;
}

// Lands the transition on the destination page: resumes playback
// position on the real video, and (conditions permitting) plays the
// flight animation from the stored rect into slotEl's real rect.
function vtLand(slotEl, videoEl) {
  if (!videoEl) return;
  var data = vtConsume();
  if (!data) return;
  ...
  var resume = function () {
    ...
    if (data.t > 0 && (!isFinite(d) || data.t < d - 0.15)) {
      try { videoEl.currentTime = data.t; } catch (e) {}
    }
  };
  if (videoEl.readyState >= 1) resume();
  else videoEl.addEventListener('loadedmetadata', resume, { once: true });

  var reduce = !!(window.matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches);
  // The stored rect only means anything if the viewport is roughly
  // the same one it was measured in (same window, no rotation).
  var sameViewport = data.rect && Math.abs(window.innerWidth - data.vw) < 80;
  if (reduce || !sameViewport || !slotEl) return;

  vtFly(slotEl, videoEl, data);
}

function vtFly(slotEl, videoEl, data) {
  // Wait a couple of frames so the destination page's own layout
  // (fonts, the slot's aspect-ratio box, etc.) has settled before we
  // measure it as the animation's end point.
  requestAnimationFrame(function () {
    requestAnimationFrame(function () {
      var endRect = slotEl.getBoundingClientRect();
      if (!endRect.width || !endRect.height) return;
      var endRadius = parseFloat(getComputedStyle(slotEl).borderRadius) || 0;
      var startRect = data.rect, startRadius = data.radius || 0;

      // The destination slot often sits inside a scroll-reveal
      // (.reveal) wrapper that's still mid fade-in on a fresh page
      // load (opacity 0 -> 1 over .7s, triggered by an
      // IntersectionObserver that hasn't necessarily resolved yet).
      // If we hand off to the real video while that's still running,
      // it visibly dips right after landing. Force it to its final
      // state instantly, with no transition — the flight itself is
      // the entrance, it doesn't need a second one layered on top.
      var revealAncestor = slotEl.closest('.reveal');
      if (revealAncestor && !revealAncestor.classList.contains('is-visible')) {
        revealAncestor.style.transition = 'none';
        revealAncestor.classList.add('is-visible');
        void revealAncestor.offsetWidth;
        revealAncestor.style.transition = '';
      }
      // Bring the rest of that hero row in alongside it (text, stats)
      // instead of leaving them to their own separately-timed
      // scroll-reveal — mirrors the source example fading in the new
      // page's content while the shared element is still in flight.
      var heroRow = slotEl.closest('.service-overview');
      if (heroRow) {
        Array.prototype.slice.call(heroRow.querySelectorAll('.reveal')).forEach(function (el, i) {
          if (el === revealAncestor || el.classList.contains('is-visible')) return;
          setTimeout(function () { el.classList.add('is-visible'); }, 120 + i * 90);
        });
      }

      // Detach the REAL video — not a clone. A clone would need its
      // own fresh buffering and show a black frame for a while, which
      // is exactly the problem we're fixing. Move it up to <body> for
      // the duration of the flight (escapes the slot's own
      // overflow:hidden / stacking context), same as the reference
      // file's lift()/land(): move the shared node out, animate,
      // move it back. A plain DOM move via appendChild doesn't
      // interrupt <video> playback.
      document.body.appendChild(videoEl);

      // Classic FLIP: size/position the element at its real final
      // (end) box, then use `transform` alone to make it *look* like
      // it's still at the start box, then animate the transform back
      // to identity. transform + border-radius are the only two
      // properties in this animation — both are cheap for the
      // browser to interpolate every frame (no layout recalculation),
      // which is what left/top/width/height animation was forcing
      // before and is why it looked jerky and "landed wrong" on
      // mobile, correcting a frame late.
      var sx = startRect.width / endRect.width;
      var sy = startRect.height / endRect.height;
      var tx = startRect.left - endRect.left;
      var ty = startRect.top - endRect.top;

      videoEl.style.position = 'fixed';
      videoEl.style.left = endRect.left + 'px';
      videoEl.style.top = endRect.top + 'px';
      videoEl.style.width = endRect.width + 'px';
      videoEl.style.height = endRect.height + 'px';
      videoEl.style.transformOrigin = 'top left';
      videoEl.style.zIndex = '2147483647';
      videoEl.style.objectFit = 'cover';
      videoEl.style.pointerEvents = 'none';
      // Dark placeholder behind the frame, so if it genuinely hasn't
      // decoded anything yet there's a deliberate-looking card in
      // flight, never a stark black rectangle.
      videoEl.style.background = 'linear-gradient(135deg,#1b2530,#10151b)';

      // Only fade if it truly has nothing to show yet — most of the
      // time (it's been loading since this document started parsing)
      // it already does, and this is a no-op.
      var needsFade = videoEl.readyState < 2;
      if (needsFade) {
        videoEl.style.opacity = '0';
        videoEl.style.transition = 'opacity .4s ease';
        videoEl.addEventListener('loadeddata', function onLoaded() {
          videoEl.style.opacity = '1';
        }, { once: true });
      }

      try { if (Math.abs(videoEl.currentTime - data.t) > 0.2) videoEl.currentTime = data.t; } catch (e) {}
      var playPromise = videoEl.play();
      if (playPromise && playPromise.catch) playPromise.catch(function () {});

      var anim = videoEl.animate([
        {
          transform: 'translate(' + tx + 'px,' + ty + 'px) scale(' + sx + ',' + sy + ')',
          borderRadius: startRadius + 'px'
        },
        { transform: 'translate(0px,0px) scale(1,1)', borderRadius: endRadius + 'px' }
      ], { duration: VT_DURATION, easing: VT_EASE, fill: 'forwards' });
      ...
      var land = function () {
        ...
        ['position', 'left', 'top', 'width', 'height', 'transform', 'transformOrigin',
          'zIndex', 'objectFit', 'background', 'pointerEvents', 'opacity', 'transition', 'borderRadius']
          .forEach(function (p) { videoEl.style[p] = ''; });
      };
      anim.onfinish = land;
      anim.oncancel = land;
    });
...
/* Offer page: capture the flag on the way out (pagehide — fires for
   the breadcrumb link, the browser's Back button, anything) and land
   the transition on the hero video if we just arrived from the
   catalog. No-ops entirely on any page that isn't an offer page. */
(function () {
  var heroVideo = document.querySelector('.service-hero-visual video');
  var heroSlot = document.querySelector('.service-hero-visual');
  if (!heroVideo || !heroSlot) return;

  vtLand(heroSlot, heroVideo);
  window.addEventListener('pagehide', function () { vtCapture(heroSlot, heroVideo); });
})();
...
(function () {
  ...
  if (svHero) {
    ...
    // Captures the on-screen rect/radius/currentTime of whichever
    // slide is visible right now, on the way out — pagehide covers a
    // click on #svLink, but also the browser's own Back/Forward
    // buttons and any other way of leaving this page, so the flight
    // isn't limited to one specific link. See vtCapture() near the
    // top of this file.
    window.addEventListener('pagehide', function () { vtCapture(svWraps[svActive], svLayers[svActive]); });
    ...
    function svRenderContent(i) {
      ...
      svLink.href = s.href;
      // Keep the URL's anchor in step with whichever slide is showing
      // (autoplay, arrows, dots, swipe — not just a manual click), so
      // the address bar always reflects/links to the current slide,
      // and a page landed on later (share, reload, this transition's
      // own svIndexFromHash()) opens on the right one.
      try {
        var slug = (s.href || '').replace(/[?#].*$/, '').replace(/\/$/, '').split('/').pop();
    ...
    /* Pause autoplay while a real pointer is hovering the slider.
       Scoped to hover-capable pointers only — on touch devices a tap
       fires a synthetic "mouseenter" with no matching "mouseleave"
       ever following it, which permanently killed the autoplay timer
       after the first touch. That's the "slides just stop changing"
       bug. */
    var svHoverCapable = !!(window.matchMedia && matchMedia('(hover: hover) and (pointer: fine)').matches);
    if (svHoverCapable) {
    ...
    // .sv-slide-video's own CSS transitions transform/opacity in over
    // 1.2-1.75s (the normal "arriving fresh" reveal) — nice on a cold
    // visit, but it would fight our own flight/land handoff if we're
    // arriving from the offer page's "back" link (the video would
    // *also* be fading in on its own schedule right as we hand off to
    // it), producing the exact opacity "dip" this is meant to avoid.
    // Skip straight to the resting state for that case.
    if (vtPeek()) {
      svWraps[0].classList.add('is-instant');
      svApplyState(0, { oy: 0, iy: 0, sx: 1, sy: 1, rot: 0, op: 1 });
      void svWraps[0].offsetWidth;
      svWraps[0].classList.remove('is-instant');
    } else {
      svApplyState(0, { oy: 0, iy: 0, sx: 1, sy: 1, rot: 0, op: 1 });
    }
    // If we just arrived here via the offer page's "back" link, pick
    // up the clip at the time it was at over there, and fly the
    // shared video in from where it was on that page.
    vtLand(svWraps[0], svLayers[0]);
...
(function () {
  ...
  /* =========================================================
     SMOOTH ("STICKY"/WEIGHTED) WHEEL SCROLL
     Same idea as the Lusion-style lerp scroll (ease the visible
     position toward a target instead of jumping straight to it),
     but applied to the page's *real* scroll position instead of a
     transform on a wrapping container — no overflow:hidden on
     <body>, no new markup needed on every template, and every
     existing scroll-driven bit of code (IntersectionObservers, the
     footer-background scroll listener above) keeps working as-is.
     Only mouse-wheel/trackpad input is smoothed; touch, keyboard
     and scrollbar-drag scrolling stay fully native. */
  (function () {
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

    var LERP = 0.09;           // 0..1 — lower feels heavier/"stickier"
    var RESYNC_PX = 2;         // if native scroll moved more than this since our last frame (keyboard, scrollbar drag, anchor jump), stop lerping and re-sync to it instead of fighting it
    ...
    function ownsItsScroll(el) {
      return !!(el && el.closest && el.closest('.nav-drawer'));
    }
    ...
    window.addEventListener('wheel', function (e) {
      ...
      if (raf === null || Math.abs(liveY - current) > RESYNC_PX) {
        current = liveY;
        target = liveY;
      }
