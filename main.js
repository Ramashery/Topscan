      ...
      if ('IntersectionObserver' in window) {
        var headingIO = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
            ...
            /* Debounce: a single-threshold observer flips back and forth
               when an element's visibility ratio hovers right at the 0.3
               line, which happens naturally during a touch drag. Each
               flip calls playZip/resetZip, and playZip forces a
               synchronous reflow (el.offsetWidth) to restart the
               animation -- rapid-fire flips near a heading meant a
               reflow storm right as you scrolled past it. Waiting a beat
               before applying a change removes that storm while still
               replaying the animation, unchanged, on every real entry. */
            if (el._zipT) clearTimeout(el._zipT);
            el._zipT = setTimeout(function () {
              el._zipT = null;
              if (el._zipOn === visible) return;
              el._zipOn = visible;
              if (visible) { playZip(el); } else { resetZip(el); }
            }, 60);
          });
        }, { threshold: 0.3 });
