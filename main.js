...
(function () {
  ...
  if (terrainWrap) {
    ...
    function startCarousel() {
      if (tvInterval || tvTotal <= 1) return;
      tvInterval = setInterval(tvSwitchNext, 4800);
    }
    ...
    document.addEventListener('visibilitychange', function () {
      if (!document.hidden && tvInView) {
        var cur = tv[tvCurrent];
        if (cur && cur.paused) cur.play().catch(function(){});
      }
    });

    /* Once this widget scrolls out of view, stop decoding/switching videos
       entirely -- nine looping <video> elements is real main-thread/decode
       cost, and there's no reason to keep paying it once the user has
       scrolled past toward the rest of the page (related services, lead
       form, footer). */
    if ('IntersectionObserver' in window) {
      new IntersectionObserver(function (entries) {
        tvInView = entries[entries.length - 1].isIntersecting;
        var cur = tv[tvCurrent];
        if (tvInView) {
          if (cur && cur.paused && !document.hidden) cur.play().catch(function(){});
          startCarousel();
        } else {
          if (cur && !cur.paused) cur.pause();
          stopCarousel();
        }
      }, { rootMargin: '25% 0px' }).observe(terrainWrap);
    }
  }
  ...
  if (!prefersReducedMotion) {
    if (!window.__zipHeroInit) {
      window.__zipHeroInit = true;
      /* ---- Hero: title -> subtitle -> contacts, chained, replaying on every viewport entry.
         Guarded so this only ever runs once: index.html and offer.html both use
         id="heroTitle"/"heroSub", and main.js concatenates the index- and offer-page
         IIFEs together on every page, so without this guard the whole sequence (zipSplit
         + its own IntersectionObserver) was being set up TWICE on both index and offer
         pages -- two observers independently forcing reflow on the same hero elements. ---- */
    ...
    /* ---- All h2 / h3: zipper animation. Plays ONCE, the first time a
       heading enters the viewport, then the observer lets go of it --
       no replay on scroll-back, so there's no repeated forced reflow
       (el.offsetWidth in playZip) fighting the scroll every time a
       heading crosses the viewport edge in either direction.
       Also guarded to run only once: main.js concatenates several
       per-page IIFEs and every one of them used to run this exact
       querySelectorAll('h2,h3') + IntersectionObserver setup, so
       every heading on every page was actually being split and
       observed three times over -- three IntersectionObservers each
       forcing their own reflow on the same elements. That tripling
       (plus, on index/offer, the same tripling of the hero title/
       subtitle animation below) was the main source of the scroll
       stutter, worst on offer pages where it also competes with two
       playing <video> elements and their parallax loop. ---- */
    if (!window.__zipHeadingsInit) {
      window.__zipHeadingsInit = true;
...
(function () {
  ...
  if (!prefersReducedMotion) {
    ...
    /* ---- Hero: title -> subtitle, chained, replaying on every viewport entry.
       Guarded so this only ever runs once: this block is duplicated (as the
       hero setup in the other page-IIFEs main.js concatenates) and every copy
       does document.getElementById('heroTitle')/('heroSub'), so on any page
       that has those ids this was being set up multiple times over -- several
       IntersectionObservers independently forcing reflow on the same elements. ---- */
    if (!window.__zipHeroInit) {
      window.__zipHeroInit = true;
      var titleDuration = 850;
  ...
  /* =========================================================
     ...
     footerTop used to come from footer.getBoundingClientRect() on
     every single scroll/rAF tick. getBoundingClientRect() forces the
     browser to flush any pending layout first -- and the site-wide
     weighted-scroll engine below writes window.scrollTo on every
     animation frame, so this handler was reading geometry back
     immediately after that write, every frame, right in the one
     region (lead form -> footer) where the footer's rect is actually
     changing. That read-after-write is forced synchronous layout,
     the classic layout-thrashing pattern, and it happened once per
     frame for as long as any part of the footer was on screen --
     which is exactly the stretch that was shaking, and why flinging
     the page (more scroll events per second -> more of these reads
     per second) made it worse. Fixed by never touching the DOM to
     read this during scroll: footer's position is measured once
     (on load/resize/content-size change) and everything else is
     plain arithmetic against window.scrollY, which never forces
     layout.
  ========================================================== */
  ...
  if (siteBg && pageFooter) {
    var lastOffset = -1;
    var ticking = false;
    var footerTopDoc = 0;   // footer's top, in document coordinates -- measured, not read every frame
    var measureFooter = function () {
      var scrollY = window.scrollY || window.pageYOffset || 0;
      footerTopDoc = pageFooter.getBoundingClientRect().top + scrollY;
    };
    var updateBgOffset = function () {
      var vh = window.innerHeight;
      var scrollY = window.scrollY || window.pageYOffset || 0;
      var footerTop = footerTopDoc - scrollY;   // same value getBoundingClientRect().top would give, no DOM read
      ...
      window.__footerCoversBg = (offset >= vh);
      ticking = false;
    };
    ...
    var onResize = function () { measureFooter(); onScrollOrResize(); };
    measureFooter();
    window.addEventListener('scroll', onScrollOrResize, { passive: true });
    window.addEventListener('resize', onResize);
    window.addEventListener('load', onResize);
    /* Re-measure if the page's height changes for a reason other than resize --
       fonts finishing, the lead-form status message appearing, a language-switch
       re-render, etc. ResizeObserver fires off the main scroll path, so this
       costs nothing during scrolling itself. */
  ...
  /* =========================================================
     FOOTER VIDEO — PAUSE WHILE OFF-SCREEN
     Unlike every other looping <video> on the site (hero, terrain
     preview), the footer's own background video had no visibility
     gating at all: autoplay+loop meant it started decoding on page
     load and never stopped, all the way through the page, on top of
     whatever else was happening as the user scrolled -- extra decode
     cost sitting right under the exact spot (footer) that was
     shaking. Paused when scrolled away, resumed when back in view,
     same as the other videos on the site. ---- */
  var footerVideo = pageFooter ? pageFooter.querySelector('.footer-media video') : null;
  if (footerVideo && 'IntersectionObserver' in window) {
    new IntersectionObserver(function (entries) {
      var inView = entries[entries.length - 1].isIntersecting;
      if (inView) { if (footerVideo.paused && !document.hidden) footerVideo.play().catch(function(){}); }
      else if (!footerVideo.paused) { footerVideo.pause(); }
    }, { rootMargin: '25% 0px' }).observe(pageFooter);
  }
  /* Shared with the particle-field canvas below: once the footer has been
     dragged all the way up (offset === vh), the fixed background sits
     fully behind it and is invisible -- there is no reason for the canvas
     to keep spending a whole frame's worth of noise + per-point drawing
     on something nobody can see. This is exactly the lead-form -> footer
     stretch of every page, so it's exactly where that wasted work showed
     up as stutter. */
  window.__footerCoversBg = false;
...
(function(){
  ...
  function frame(now){
    ...
    /* Fully hidden behind the footer right now (the lead-form -> footer
       stretch of the page) -- skip the noise field and the per-point draw
       entirely rather than spending a frame on invisible pixels. The rAF
       call above stays so the field picks back up the instant the footer
       scrolls back down and it becomes visible again. */
    if (window.__footerCoversBg) return;
    /* Actively mid-scroll (the weighted-scroll engine is still easing
       toward its target) -- this field is a slow, ambient background
       motion with nothing tying it to scroll position, so pausing it
       for the always-brief duration of a scroll gesture is invisible,
       while the ~3-10ms/frame it otherwise costs (noise sample + arc +
       fill per point, hundreds of points) is exactly the main-thread
       budget the scroll engine needs on every frame to keep window.scrollTo
       and the touchmove handling smooth. The harder/faster you scroll,
       the more of these frames land mid-gesture, which is why forcing
       the scroll made the stutter worse: this field was still spending
       its full budget on every one of them. */
    if (window.__activeScrolling) return;
...
(function () {
  ...
  if (footerContacts) {
    ...
    if ('IntersectionObserver' in window) {
      var fcT = null, fcOn = null;
      var footerContactsIO = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          var visible = entry.isIntersecting;
          if (fcT) clearTimeout(fcT);
          fcT = setTimeout(function () {
            fcT = null;
            if (fcOn === visible) return;
            fcOn = visible;
            if (visible) { playBurst(footerContacts); } else { resetBurst(footerContacts); }
          }, 60);
        });
      }, { threshold: 0.3 });
  ...
  (function () {
    ...
    /* document.documentElement.scrollHeight forces the browser to flush any
       pending layout before it can answer -- calling it from moveTarget()
       below, on every single wheel/touchmove/keydown event, meant a forced
       layout read right in the middle of a fast swipe (touchmove fires at
       native touch rate, so a forceful fling could mean this read dozens of
       times a second) immediately after tick()'s own window.scrollTo write.
       That read-after-write is exactly what made flinging/forcing the scroll
       feel worse, not better. It's measured once instead, and only
       re-measured on resize or when the page's own height changes. */
    ...
    function tick(now) {
      ...
      if (Math.abs(diff) < 0.4) {
        ...
        raf = null;
        window.__activeScrolling = false;
        return;
      }
      current += diff * k;
      window.scrollTo({ top: current, left: 0, behavior: 'instant' });
    ...
    function kick() { if (raf === null) { last = 0; window.__activeScrolling = true; raf = requestAnimationFrame(tick); } }
    function resync() {
    ...
    if ('ResizeObserver' in window) {
      new ResizeObserver(computeMaxScroll).observe(document.body);
    }
  })();
  ...
  (function () {
    ...
    // main thread. This loop used to also call getBoundingClientRect() on every
    // box on every frame to find its position -- a forced-layout read stacking
    // on top of that decode cost, worst right when the weighted-scroll engine
    // was also writing window.scrollTo every frame (a write immediately
    // followed by a read is what forces the browser to do the layout
    // synchronously instead of on its own schedule). Each box's position is now
    // measured once -- from the IntersectionObserver's own free boundingClientRect,
    // or on resize -- and the per-frame tick is plain arithmetic against
    // window.scrollY, so scrolling no longer forces any layout here at all.
    // On touch devices we still update video parallax at 30fps instead of 60fps:
    // the shift itself is slow and subtle, so the halved rate isn't visible, but
    // it does halve the work competing with video decode during a scroll.
    ...
    function tick() {
      ...
      visible.forEach(function (it) {
        ...
        var top = it.topDoc - scrollY;
        var center = top + it.height / 2;
    ...
    if ('IntersectionObserver' in window) {
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          if (!it) return;
          var r = entry.boundingClientRect;   // free with the observer, no extra layout read
          var scrollY = window.scrollY || window.pageYOffset || 0;
          it.topDoc = r.top + scrollY;
          it.height = r.height;
