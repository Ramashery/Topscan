...
/* =========================================================
   ...
   Hand-rolled FLIP-style flight, NOT the native cross-document View
   Transition API. That API turned out to be the reason the previous
   version of this feature was unreliable: whether a given navigation
   actually gets a transition depends on browser support (Chrome/Edge
   only) plus internal timing of the Navigation API that isn't fully
   in our control — on some clicks it simply never engaged, which is
   why the effect "worked sometimes" or not at all depending on the
   browser/device.

   This version does the whole thing itself with the Web Animations
   API, which every current browser (incl. mobile Safari) supports,
   so it now works the same way everywhere:

   1. On click (services -> offer via #svLink, offer -> services via
      the breadcrumb's "Services" link), vtCapture() measures the
      on-screen rect + border-radius of the current "slot" (the full
      -bleed .sv-slide on the catalog page, .service-hero-visual on
      the offer page) and stores it in sessionStorage together with
      the video's src and currentTime. The browser then navigates
      normally — nothing is intercepted or delayed.
   2. On the destination page, vtLand() reads that back, resumes the
      real <video> at the stored currentTime (so the clip carries on
      instead of restarting at 0), and — unless reduced motion is on
      or the viewport changed too much to make the stored rect
      meaningful — plays a short-lived cloned <video> as a
      position:fixed overlay that flies/resizes from the stored rect
      into the real slot's rect, then hands off to the real video
      underneath. That's the whole "shared element" illusion.
========================================================== */
var VT_KEY = 'topscanVT';
var VT_DURATION = 680;
var VT_EASE = 'cubic-bezier(.76,0,.24,1)';

// Called on click of a transition trigger link, on the "slot" (the
// element whose rect/radius represents the shared video's visible
// shape) and the actual <video> inside it.
...
function vtFly(slotEl, videoEl, data) {
  ...
  requestAnimationFrame(function () {
    requestAnimationFrame(function () {
      var targetRect = slotEl.getBoundingClientRect();
      if (!targetRect.width || !targetRect.height) return;
      var targetRadius = parseFloat(getComputedStyle(slotEl).borderRadius) || 0;
      ...
      // Wrapper handles position/size/radius/background (a dark
      // placeholder so there's never a stark black flash while the
      // clip re-buffers); the video inside only fades in once it
      // actually has a frame to show. Same split as the reference
      // file's .shared / .shared video — geometry and "is this
      // actually playable yet" are two separate concerns there too.
      var wrap = document.createElement('div');
      wrap.setAttribute('aria-hidden', 'true');
      wrap.style.cssText = 'position:fixed;top:0;left:0;overflow:hidden;z-index:2147483647;'
        + 'pointer-events:none;background:linear-gradient(135deg,#16242d,#0c151b);'
        + 'will-change:left,top,width,height,border-radius;';
      wrap.style.left = data.rect.left + 'px';
      wrap.style.top = data.rect.top + 'px';
      wrap.style.width = data.rect.width + 'px';
      wrap.style.height = data.rect.height + 'px';
      wrap.style.borderRadius = data.radius + 'px';
      ...
      overlay.src = data.src;
      overlay.style.cssText = 'display:block;width:100%;height:100%;object-fit:cover;'
        + 'opacity:0;transition:opacity .5s ease;';
      wrap.appendChild(overlay);
      document.body.appendChild(wrap);

      var startPlay = function () {
        try { overlay.currentTime = data.t; } catch (e) {}
        var p = overlay.play();
        if (p && p.catch) p.catch(function () {});
      };
      if (overlay.readyState >= 1) startPlay();
      else overlay.addEventListener('loadedmetadata', startPlay, { once: true });

      // Geometry starts moving right away (feels responsive); the
      // clip itself only becomes visible once there's an actual
      // frame decoded, instead of animating a black rectangle.
      var fadeIn = function () { overlay.style.opacity = '1'; };
      if (overlay.readyState >= 2) fadeIn();
      else overlay.addEventListener('loadeddata', fadeIn, { once: true });

      var prevVisibility = videoEl.style.visibility;
      videoEl.style.visibility = 'hidden';

      var anim = wrap.animate([
        { left: data.rect.left + 'px', top: data.rect.top + 'px', width: data.rect.width + 'px', height: data.rect.height + 'px', borderRadius: data.radius + 'px' },
        { left: targetRect.left + 'px', top: targetRect.top + 'px', width: targetRect.width + 'px', height: targetRect.height + 'px', borderRadius: targetRadius + 'px' }
      ], { duration: VT_DURATION, easing: VT_EASE, fill: 'forwards' });

      var land = function () {
        videoEl.style.visibility = prevVisibility;
        try { if (overlay.currentTime) videoEl.currentTime = overlay.currentTime; } catch (e) {}
        wrap.remove();
      };
      anim.onfinish = land;
      anim.oncancel = land;
...
/* Offer page: hook the breadcrumb's "Services" link up to the
   mechanic above, and land the transition on the hero video if we
   just arrived from the catalog. No-ops entirely on any page that
   isn't an offer page. */
(function () {
  ...
  var breadcrumb = document.getElementById('pageBreadcrumb');
  var backLink = breadcrumb && breadcrumb.querySelector('a[href*="/services"]');
  if (backLink) {
    backLink.addEventListener('click', function () { vtCapture(heroSlot, heroVideo); });
  }
})();
...
(function () {
  ...
  if (svHero) {
    ...
    if (svLink) {
      // Captures the on-screen rect/radius/currentTime of the slide
      // that's visible right now, right before the browser navigates
      // to the offer page. See vtCapture() near the top of this file.
      svLink.addEventListener('click', function () { vtCapture(svWraps[svActive], svLayers[svActive]); });
    }
