...
/* =========================================================
   ...
   the code below narrows that down to just the services-catalog
   <-> offer pair, and makes the shared video resume at the exact
   time it was at when the user navigated away, instead of
   restarting at 0.

   Eligibility is decided by an explicit flag written to
   sessionStorage at the moment one of the two trigger links is
   clicked (see the services-slider code further down, and the
   offer-page breadcrumb hookup right below) — NOT by inspecting
   the navigation's URL or document.referrer from inside the
   pageswap/pagereveal handlers. That URL-guessing approach is what
   caused the effect to fire inconsistently: document.referrer can
   come back empty (stripped by the browser's referrer policy,
   private-browsing, some in-app browsers) and the Navigation API's
   `activation` isn't reliably populated at the exact moment these
   two events fire on every browser/timing. Reading a flag we set
   ourselves, synchronously, in the click handler removes that
   guesswork entirely.
========================================================== */
var VT_KEY = 'topscanVT';

// Called on click, on the currently-playing shared video, right
// before the browser navigates away.
function vtMark(videoEl) {
  try {
    sessionStorage.setItem(VT_KEY, JSON.stringify({
      t: (videoEl && isFinite(videoEl.currentTime)) ? videoEl.currentTime : 0,
      ts: Date.now()
    }));
  } catch (e) {}
}

// Peeks at the flag without consuming it. maxAgeMs guards against a
// stale flag surviving into some unrelated, much later navigation.
function vtRead(maxAgeMs) {
  try {
    var raw = sessionStorage.getItem(VT_KEY);
    if (!raw) return null;
    var data = JSON.parse(raw);
    if (!data || (Date.now() - data.ts) > maxAgeMs) return null;
    return data;
  } catch (e) { return null; }
}

// Resumes `videoEl` at the timestamp the previous page's shared
// video was at, so the clip carries on instead of restarting at 0.
// Consumes (clears) the flag, so it's only ever applied once.
function vtApplyContinuity(videoEl) {
  if (!videoEl) return;
  var data = vtRead(4000);
  if (!data) return;
  try { sessionStorage.removeItem(VT_KEY); } catch (e) {}
  var apply = function () {
    var d = videoEl.duration;
    if (isFinite(data.t) && data.t > 0 && (!isFinite(d) || data.t < d - 0.15)) {
...
(function () {
  ...
  window.addEventListener('pageswap', function (e) {
    ...
    if (reduceMotion || !vtRead(2000)) e.viewTransition.skipTransition();
  });

  // Fired on the page being navigated TO, right before it's shown.
  window.addEventListener('pagereveal', function (e) {
    if (!e.viewTransition) return;
    if (reduceMotion || !vtRead(4000)) e.viewTransition.skipTransition();
  });
})();

/* Offer page: hook the breadcrumb's "Services" link up to the same
   mechanic (marks the flag on click) and resume the hero video at
   the time the services-catalog slide was at (in case the user
   arrived here from there). No-ops entirely on any page that isn't
   an offer page. */
(function () {
  var heroVideo = document.querySelector('.service-hero-visual video');
  if (!heroVideo) return;

  vtApplyContinuity(heroVideo);

  var breadcrumb = document.getElementById('pageBreadcrumb');
  var backLink = breadcrumb && breadcrumb.querySelector('a[href*="/services"]');
  if (backLink) {
    backLink.addEventListener('click', function () { vtMark(heroVideo); });
  }
})();
...
(function () {
  ...
  if (svHero) {
    ...
    if (svLink) {
      // Marks the flag (currentTime of the clip on screen right now)
      // right before the browser navigates to the offer page.
      svLink.addEventListener('click', function () { vtMark(svLayers[svActive]); });
    }
    ...
    // up the clip at the time it was at over there instead of 0:00.
    vtApplyContinuity(svLayers[0]);
