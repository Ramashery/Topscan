...
(function () {
  ...
  var slotMain = $('#abSlotMain'), skipBtn = $('#abSkip');
  ...
  /* =====================================================================
     SHARED-ELEMENT TRANSITION
       ...
       3. hero overlay (breadcrumb, scrim, corners, scroll cue) fades out;
  ...
  /* intro -> page triggers: HERO_MS after playback starts, or the visitor asks to move on */
  var heroTimer = null;
  ...
  if (skipBtn) skipBtn.addEventListener('click', goAbout);
...
/* =========================================================
   ...
   overview window, run in the other direction. The bio then appears
   at the bottom. Back (button, Android back, Escape) reverses it.
   The tap also adds a history entry, so the system Back button
   returns to the grid instead of leaving the page.
========================================================== */
(function () {
  'use strict';
  ...
  var stageMain = $('#abStageMain');
  var detail = $('#abDetail'), backBtn = $('#abBack'), bodyEl = $('#abDetailBody');
  ...
  if (!stageMain || !detail || !backBtn || !articles.length) return;
  ...
  async function openMember(article, opts) {
    ...
    clearBox(photo);
    photo.classList.add('is-full');

    void detail.offsetWidth;                            /* let the fade-in transition run */
  ...
  async function closeMember(opts) {
    ...
    detail.classList.remove('is-open');
    if (!opts.instant && !reduceMotion) await wait(240);

    /* restore the page scroll BEFORE measuring where the card is */
    if (window.__unlockBodyScroll) window.__unlockBodyScroll();
    stageMain.inert = false;

    var from = rectOf(photo), to = rectOf(c.ph);
    photo.classList.remove('is-full');
    Object.assign(photo.style, frame(from));
  ...
  /* Back button: go through history when we pushed an entry, so the URL and the system Back stay in sync */
