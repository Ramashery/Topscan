...
def build_site(data, lang):
    s = data["settings"]
    s_t = loc(s, "en")  # legal/footer copy stays in English on every language version
    return {
        "phone":          s.get("phone", ""),
...
def build_footer_labels(data, lang):
    # Footer copy is intentionally NOT localized (client request: footer
    # stays in English on every language version).
    s_t = loc(data["settings"], "en")
    return {
...
def copy_static():
    ...
    for name in os.listdir("."):
        if name in COPY_IGNORE or name.startswith(".") or name.startswith("_"):
            continue
