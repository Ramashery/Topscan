...
URL scheme (no trailing slash anywhere except the bare domain):
  ...
  /en/works                   -> works (case studies) catalog
  /en/works/{slug}              -> a single case-study ("cases") page
...
COPY_IGNORE = {
    ...
    "README.md", "seed.html", "seed_about.html", "seed_works.html",
    "about-2.html",   # the static draft — replaced by tpl_about.html
    "tpl_index.html", "tpl_services.html", "tpl_offer.html", "tpl_about.html", "tpl_works.html", "tpl_cases.html",
...
try:
    ...
    T = {
        ...
        "works":    jinja.get_template("tpl_works.html"),
        "cases":    jinja.get_template("tpl_cases.html"),
    }
    print("Templates OK")
except Exception as e:
...
def load_firestore():
    ...
    data["page_works"]    = db.collection("pages").document("works").get().to_dict() or {}
    ...
    for doc in db.collection("cases").stream():
        ...
        if (d.get("status") or "").lower() != "published":
            continue
        cases.append(d)
    cases.sort(key=lambda c: (c.get("order") if c.get("order") is not None else 999))
    data["cases"] = cases

    print(f"  settings, leadForm, pages/home, pages/services, pages/about, pages/works loaded")
    print(f"  services: {len(services)} published")
    print(f"  team: {len(team)} published")
    print(f"  cases: {len(cases)} published")
    return data
...
def case_industry_label(case, lang):
    # Badge text ("Construction" / "Municipal & GIS" / "Agriculture") is
    # intentionally NOT localized — same client rule as category_label()
    # for services — always read from the English translation.
    case_t = loc(case, "en")
    return case_t.get("industryLabel") or case_t.get("categoryLabel") or case.get("industry", "")


def case_summary(case, lang):
    """Compact per-language view of a case study, used in the works-catalog
    slider and in "related cases" cards."""
    t = loc(case, lang)
    return {
        "slug": case["slug"],
        "title": t.get("title", ""),
        "cardDesc": t.get("cardDesc", ""),
        "bullets": t.get("bullets", []),
        "categoryLabel": case_industry_label(case, lang),
...
def related_cases_for(case, all_cases, lang, limit=3):
    slugs = case.get("relatedSlugs")
    by_slug = {c["slug"]: c for c in all_cases}
    if slugs:
        chosen = [by_slug[s] for s in slugs if s in by_slug]
    else:
        chosen = [c for c in all_cases
                  if c["slug"] != case["slug"] and c.get("industry") == case.get("industry")]
    return [case_summary(c, lang) for c in chosen[:limit]]
...
def gen_works_catalog(data):
    print("--- Works catalog ---")
    sitemap = []
    if not data["cases"]:
        print("  ! no published documents in 'cases' — skipping. Run seed_works.html once.")
        return sitemap
    for lang in SUPPORTED_LANGS:
        t = loc(data["page_works"], lang)
        t_en = loc(data["page_works"], "en")
        t = anglicize(t, t_en, ["learnMore"])
        footer_labels = build_footer_labels(data, lang)
        works_view = [case_summary(c, lang) for c in data["cases"]]
        # Key names (cat/title/desc/bullets/href/video) match the shared
        # slider engine in main.js — see the comment block in tpl_works.html.
        works_json = json.dumps([
            {
                "cat": w["categoryLabel"], "title": w["title"], "desc": w["cardDesc"],
                "bullets": w["bullets"], "href": f"{SITE_PREFIX}/{lang}/works/{w['slug']}", "video": w["video"],
            }
            for w in works_view
        ], ensure_ascii=False)
        ctx = base_ctx(data, lang, "works", lambda l: f"/{l}/works")
        ctx["t"] = {**t, **footer_labels}
        ctx["works"] = works_view
        ctx["works_json"] = works_json
        html = T["works"].render(**ctx)
        write(f"{OUTPUT_DIR}/{lang}/works.html", html)
        sitemap.append({"path": f"/{lang}/works", "lang": lang})
    print(f"  {len(SUPPORTED_LANGS)} language(s) written ({len(data['cases'])} case(s))")
    return sitemap


# Generic section copy for the case-detail page. Case documents only carry
# each section's `eyebrow` (translated per case, forced to English below —
# same "badge text stays English" rule as included.eyebrow/process.eyebrow
# on the offer page) plus the actual data; the H2 wording is identical on
# every case, so — like relatedEyebrow/relatedTitle/learnMore on the offer
# page — it lives here as a plain constant rather than in Firestore.
CASE_SECTION_DEFAULTS = {
    "challengeTitle":     "Site & Client",
    "requiredAccuracy":   "Required accuracy",
    "flightParamsTitle":  "Flight Parameters",
    "accuracyQaTitle":    "Accuracy QA/QC",
...
def gen_case_detail(data):
    print("--- Case (works) detail pages ---")
    sitemap = []
    count = 0
    works_t_en = loc(data["page_works"], "en")
    for case in data["cases"]:
        slug = case["slug"]
        for lang in SUPPORTED_LANGS:
            case_t = loc(case, lang)
            case_t_en = loc(case, "en")
            case_t = anglicize(case_t, case_t_en, [
                "challenge.eyebrow", "flightParams.eyebrow", "accuracyQa.eyebrow",
                "businessValue.eyebrow",
            ])
            footer_labels = build_footer_labels(data, lang)
            works_t = loc(data["page_works"], lang)
            # hudStats (area/turnaround/accuracy) comes straight from the
            # doc; equipment + RTK status are top-level, non-translated
            # fields (same "instrument name" nature as service.video), so
            # they're appended here as two more ready-made stat entries
            # instead of asking every case document to repeat them 4x.
            hud_stats = list(case_t.get("hudStats") or [])
            hud_stats.append({"value": case.get("equipment", ""), "label": "Equipment"})
            hud_stats.append({
                "value": "RTK FIX · ON" if case.get("rtkFix") else "RTK FIX · OFF",
                "label": "Survey status",
            })
            t = {
                **case_t,
                **footer_labels,
                "industryLabel": case_industry_label(case, lang),
                # Same non-localized-breadcrumb rule as the offer page.
                "breadcrumbTitle": case_t_en.get("shortTag") or case_t_en.get("title", ""),
                "hudStats": hud_stats,
                "deliverablesLegend": works_t.get("deliverablesLegend", {}),
                "learnMore": case_t.get("learnMore") or works_t_en.get("learnMore") or "View case study",
                **CASE_SECTION_DEFAULTS,
            }
            ctx = base_ctx(data, lang, "works", lambda l, slug=slug: f"/{l}/works/{slug}")
            ctx["t"] = t
            ctx["case"] = {
                "slug": slug,
                "video": case.get("video", ""),
                ...
                "rtkFix": bool(case.get("rtkFix")),
                "deliverables": case.get("deliverables", []),
            }
            ctx["related_cases"] = related_cases_for(case, data["cases"], lang)
            html = T["cases"].render(**ctx)
            write(f"{OUTPUT_DIR}/{lang}/works/{slug}.html", html)
            sitemap.append({"path": f"/{lang}/works/{slug}", "lang": lang})
            count += 1
    if not data["cases"]:
        print("  ! no published documents in 'cases' — skipping.")
    else:
        print(f"  {count} page(s) written ({len(data['cases'])} cases x {len(SUPPORTED_LANGS)} langs)")
    return sitemap
...
def main():
    ...
    sitemap += gen_works_catalog(data)
    sitemap += gen_case_detail(data)
