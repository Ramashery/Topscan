...
COPY_IGNORE = {
    ...
    "tpl_index.html", "tpl_services.html", "tpl_offer.html", "tpl_about.html", "tpl_works.html",
...
def case_summary(case, lang):
    ...
    slider. Mirrors svc_summary(); `industry` uses the same fallback labels
    as a service's `category: industry` (see DEFAULT_CATEGORY_LABELS) since
    a case's HUD header ("ОТРАСЛЬ / INDUSTRY") is the same three values:
    construction / municipal_gis / agriculture."""
    t = loc(case, lang)
    ...
    industry_label = loc(case, "en").get("categoryLabel") or case.get("industry", "")
    return {
        "slug": case["slug"],
        "title": t.get("title", ""),
        "cardDesc": t.get("cardDesc", ""),
        "bullets": t.get("bullets", []),
        "categoryLabel": industry_label,
